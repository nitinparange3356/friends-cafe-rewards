...
const defaultCatImage = "https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?w=100&h=100&fit=crop";
      ...
      <div className="sticky top-14 z-40 bg-card border-b md:hidden">
        ...
        <div className="flex items-center gap-2 px-3 pb-2 overflow-x-auto no-scrollbar">
          {(["veg", "non-veg", "egg"] as VegType[]).map((type) => {
            ...
            return (
              <button key={type} onClick={() => setVegFilter(isActive ? null : type)}
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-xs font-medium flex-shrink-0 transition-all ${isActive ? `${colors[type]} bg-card shadow-sm` : "border-border"}`}>
                <div className={`w-3 h-3 border-[1.5px] ${colors[type]} flex items-center justify-center rounded-sm`}><div className={`w-1.5 h-1.5 rounded-full ${dotColors[type]}`} /></div>
                <span className="capitalize">{type}</span>
              </button>
            );
          })}
          <button onClick={() => setVegFilter(null)}
            className={`px-2.5 py-1 rounded-full border text-xs font-medium flex-shrink-0 transition-all ${!vegFilter ? "border-foreground bg-foreground text-background" : "border-border"}`}>All</button>
        </div>
      </div>
        ...
        <aside className="hidden md:block w-36 lg:w-44 flex-shrink-0 border-r bg-card sticky top-14 h-[calc(100vh-3.5rem)] overflow-y-auto">
          <div className="py-2">
            <button onClick={() => setActiveCategory("All")}
              className={`w-full flex flex-col items-center gap-1 py-3 px-2 text-xs font-semibold transition-colors relative ${activeCategory === "All" ? "text-primary" : "text-muted-foreground hover:text-foreground"}`}>
        ...
        <main className="flex-1 min-w-0">
          <div className="hidden md:flex items-center gap-3 px-6 py-3 border-b bg-card">
            {(["veg", "non-veg", "egg"] as VegType[]).map((type) => {
              ...
              return (
                <button key={type} onClick={() => setVegFilter(isActive ? null : type)}
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-full border-2 text-sm font-medium transition-all ${isActive ? `${colors[type]} bg-card shadow-sm` : "border-border hover:border-muted-foreground/30"}`}>
                  <div className={`w-3.5 h-3.5 border-2 ${colors[type]} flex items-center justify-center rounded-sm`}><div className={`w-1.5 h-1.5 rounded-full ${dotColors[type]}`} /></div>
                  <span className="capitalize">{type}</span>
                </button>
              );
            })}
            <button onClick={() => setVegFilter(null)}
              className={`px-3 py-1.5 rounded-full border-2 text-sm font-medium transition-all ${!vegFilter ? "border-foreground bg-foreground text-background" : "border-border hover:border-muted-foreground/30"}`}>All</button>
          </div>
